...
public class AUMultiField {
	...
	public void flattenRGBA(float _fr, float _fg, float _fb, float _fa) {
		if (fields.length < 4) {
			AULib.reportError("AUMultiField", "flattenRGBA", "less than 4 fields avialable",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		for (int y = 0; y < h; y++) {
			for (int x = 0; x < w; x++) {
	...
	public void flattenRGB(float _fr, float _fg, float _fb) {
		if (fields.length < 3) {
			AULib.reportError("AUMultiField", "flattenRGB", "less than 3 fields avialable",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		for (int y = 0; y < h; y++) {
	 ...
	 * _pg.height; _pg.loadPixels(); thesePixels = _pg.pixels; } else { wid =
	 * theSketch.width; hgt = theSketch.height; theSketch.loadPixels(); thesePixels
	 * = theSketch.pixels; } for (int y=0; y<h; y++) { if (y >= hgt) continue; for
	 * (int x=0; x<w; x++) { if (x >= wid) continue; int c = thesePixels[(y*wid)+x];
	 * fields[0].z[y][x] = (c>>16) & 0xFF;//AUMisc.jred(c); fields[1].z[y][x] =
	 * (c>>8) & 0xFF;//AUMisc.jgreen(c); fields[2].z[y][x] = c &
	 * 0xFF;//AUMisc.jblue(c); if (_saveAlpha) fields[3].z[y][x] = (c>>24) &
	 * 0xFF;//AUMisc.jalpha(c); } } }
	 */
	void loadFromPixels(boolean _saveAlpha, int[] _pixels, int _pixwid, int _pixhgt) {
		...
		if (fields.length < fieldsNeeded) {
			AULib.reportError("AUMultiField", "RGBAfromPixels", "there are not at least " + fieldsNeeded + " fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		int wid = _pixwid;
		...
		for (int y = 0; y < h; y++) {
			...
			for (int x = 0; x < w; x++) {
				...
				if (_saveAlpha) {
					fields[3].z[y][x] = (c >> 24) & 0xFF;// AUMisc.jalpha(c);
				}
			}
		}
	}
	...
	public void RGBAadd(float _fr, float _fg, float _fb, float _fa) {
		if (fields.length < 4) {
			AULib.reportError("AUMultiField", "RGBAadd", "there are not at least 4 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		fields[0].add(_fr);
	...
	public void RGBadd(float _fr, float _fg, float _fb) {
		if (fields.length < 3) {
			AULib.reportError("AUMultiField", "RGBadd", "there are not at least 3 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		fields[0].add(_fr);
	...
	public void RGBAmul(float _mr, float _mg, float _mb, float _ma) {
		if (fields.length < 4) {
			AULib.reportError("AUMultiField", "RGBAmul", "there are not at least 4 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		fields[0].mul(_mr);
	...
	public void RGBmul(float _mr, float _mg, float _mb) {
		if (fields.length < 3) {
			AULib.reportError("AUMultiField", "RGBmul", "there are not at least 3 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		fields[0].mul(_mr);
	...
	public void setTriple(int _x, int _y, float _v0, float _v1, float _v2) {
		if ((_x < 0) || (_x >= w) || (_y < 0) || (_y >= h)) {
			AULib.reportError("AUMultiField", "setTriple", "the point x,y is beyond the field's bounds",
					"x=" + Float.toString(_x) + " y=" + Float.toString(_y));
		}
		if (fields.length < 3) {
			AULib.reportError("AUMultiField", "setTriple", "there are not at least 3 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
	...
	public void setQuad(int _x, int _y, float _v0, float _v1, float _v2, float _v3) {
		if ((_x < 0) || (_x >= w) || (_y < 0) || (_y >= h)) {
			AULib.reportError("AUMultiField", "setQuad", "the point x,y is beyond the field's bounds",
					"x=" + Float.toString(_x) + " y=" + Float.toString(_y));
			return;
		}
		if (fields.length < 4) {
			AULib.reportError("AUMultiField", "setQuad", "there are not at least 4 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		fields[0].z[_y][_x] = _v0;
	...
	public void RGBAsetColor(int _x, int _y, int _c) {
		if (fields.length < 4) {
			AULib.reportError("AUMultiField", "RGBAsetColor", "there are not at least 4 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		int ired = (_c >> 16) & 0xFF;
		...
		// setQuad(_x, _y, AUMisc.jred(_c), AUMisc.jgreen(_c), AUMisc.jblue(_c),
		// AUMisc.jalpha(_c));
	}

	public void RGBsetColor(int _x, int _y, int _c) {
		if (fields.length < 3) {
			AULib.reportError("AUMultiField", "setColor", "there are not at least 3 fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		int ired = (_c >> 16) & 0xFF;
	...
	int getColor(int _x, int _y, boolean _getAlpha) {
		...
		if (fields.length < fieldsNeeded) {
			AULib.reportError("AUMultiField", "setColor", "there are not at least " + fieldsNeeded + " fields available",
					"fields.length=" + Float.toString(fields.length));
			return 0;
		}
	...
	// This is very much like writeToPixels, but for efficiency I'm repeating it
	// here for compositions between
	...
	// Write out the pixels in this field to a destination. The upper-left corner of
	// the field will appear
	// at (_dx, _dy) in the destination. The mask provides alpha blending. There's a
	// boolean that lets you
	// use the 4th channel (that is, layer 3) as a mask, as usual for RGBA images.
	// The mask is aligned to
	// the destination coordinate system; it is not aligned to the source image. The
	// upper-left corner of
	// the mask is place at (_mx, _my). Finally, the rectangle of destination pixels
	// if of size
	// (_destWid, _destHgt) and the contents are in the pixel array _destPixels,
	// packed as 8 bits each in
	// order RGBA into a 24-bit int in standard Processing format: ARGB (A in the
	// MSB, B in the LSB).

	void writeToPixels(int _dx, int _dy, AUField _mask, boolean _useLayer3AsMask, int _mx, int _my, int[] _destPixels, int _destWid,
			int _destHgt) {
		if ((_mask != null) && (_useLayer3AsMask)) {
			AULib.reportError("AUMultiField", "writeToPixels",
					"you supplied a mask AND said to use layer 3 as alpha. Ignoring layer 3 and using the mask.", "");
		...
		if (fields.length < fieldsNeeded) {
			AULib.reportError("AUMultiField", "writeToPixels", "there are not at least " + fieldsNeeded + " fields available",
					"fields.length=" + Float.toString(fields.length));
			return;
		}
		int h = fields[0].h;
		...
		if (_mask != null) {
			thisMask = _mask;
		} else if (_useLayer3AsMask) {
			thisMask = fields[3];
		}
		for (int y = 0; y < h; y++) {
			int destY = y + _dy;
			if ((destY < 0) || (destY >= _destHgt)) {
				continue; // outside the target bounds?
			}
			for (int x = 0; x < w; x++) {
				int destX = x + _dx;
				if ((destX < 0) || (destX >= _destWid)) {
					continue; // outside the target bounds?
				}
				...
				// destIndex="+destIndex+" _destWid="+(_destWid)+" _destHgt="+(_destHgt)+"
				// w="+w+" h="+h);
				...
				float blendP = 1.f;
				// as with AUField, this is superfluous now that the write calls are unified,
				// but leaving it in for a while
				// if (_pg != null) { // the screen is always opaque, but PGraphics might not be
				// blendF = fieldAlpha;
				// blendP = pixelAlpha; // * (1.f - fieldAlpha); <- Alvy's scale factor not
				// needed any more
				// }
	...
	// And now the 12 different ways we can write to pixels! There are four basic
	// calls:
	// 1. write RGB to this upper-left
	// 2. write RGBA to this upper-left
	// 3. write RGB to this upper-left with this mask
	// 4. write RGB to this upper-left with this mask, which is offset by these
	// values
	// I don't include writing RGBA when a mask is present, because the mask
	// provides the
	...
	// So that's a total of 12 calls. They just call writeToPixels() with slightly
	// different arguments.

	public void RGBtoPixels(float _dx, float _dy) { // copy opaque RGB values into pixels
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), null, false, Math.round(_dx), Math.round(_dy), theSketch.pixels, theSketch.width,
				theSketch.height);
	...
	public void RGBAtoPixels(float _dx, float _dy) { // copy RGBA values into pixels
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), null, true, Math.round(_dx), Math.round(_dy), theSketch.pixels, theSketch.width,
				theSketch.height);
		theSketch.updatePixels();
	}
	...
	public void RGBtoPixels(float _dx, float _dy, AUField _mask) { // use mask to mix in RGB
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, false, Math.round(_dx), Math.round(_dy), theSketch.pixels, theSketch.width,
				theSketch.height);
		theSketch.updatePixels();
	}
	...
	public void RGBtoPixels(float _dx, float _dy, AUField _mask, float _mx, float _my) { // use mask to mix in RGB
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, false, Math.round(_mx), Math.round(_my), theSketch.pixels, theSketch.width,
				theSketch.height);
		theSketch.updatePixels();
	}
	...
	public void RGBtoPixels(float _dx, float _dy, PImage _img) { // copy opaque RGB values into pixels
		_img.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), null, false, Math.round(_dx), Math.round(_dy), _img.pixels, _img.width,
				_img.height);
		_img.updatePixels();
	}
	...
	public void RGBtoPixels(float _dx, float _dy, AUField _mask, PImage _img) { // use mask to mix in RGB
		_img.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, false, Math.round(_dx), Math.round(_dy), _img.pixels, _img.width,
				_img.height);
		_img.updatePixels();
	}
	...
	public void RGBtoPixels(float _dx, float _dy, AUField _mask, float _mx, float _my, PImage _img) { // use mask to mix in RGB
		_img.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, false, Math.round(_mx), Math.round(_my), _img.pixels, _img.width,
				_img.height);
		_img.updatePixels();
	}
}
