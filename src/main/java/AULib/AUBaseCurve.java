...
public class AUBaseCurve {
	...
	// I don't do validity checking in these constructors because the two
	...
	/*
	 * We need this because subclasses have to call the superclass constructor on
	 * their first line, but they have to build the knots array from their
	 * arguments. So they call the constructor with everything it needs but the
	 * knots, and then fill in the knots later.
	 */
	...
	public float getIndexValue(float _t, int _index) {
		...
		float v = evalCurve(knots[mappedKnotStart][_index], knots[mappedKnotStart + 1][_index], knots[mappedKnotStart + 2][_index],
				knots[mappedKnotStart + 3][_index]);
		return v;
	}
	...
	public float getIndexTan(float _t, int _index) {
		...
		float v = evalTan(knots[mappedKnotStart][_index], knots[mappedKnotStart + 1][_index], knots[mappedKnotStart + 2][_index],
				knots[mappedKnotStart + 3][_index]);
		return v;
	}
	...
	public void setX(int _knotNum, float _x) {
		setKnotIndex(_knotNum, 0, _x);
	}

	public void setY(int _knotNum, float _y) {
		setKnotIndex(_knotNum, 1, _y);
	}

	public void setZ(int _knotNum, float _z) {
	...
	// To find [t^3 t^2 t 1] M [v0 v1 v2 v3]T, this finds v=([t^3 t^2 t 1] M) and
	// saves it in global leftSide.
	// We split this off because we need to do this only once to evaluate multiple
	// data values at a given t.
	...
	/*
	 * Binary subdivision through ArcLengths to find the global value of t. This
	 * returns nothing, but side-effects three globals: mappedT the t in the found
	 * segment mappedSegment the found segment
	 */
	void mapTtoCurve(float t) {
		...
		if (t < 0.) {
			t = (float) (Math.ceil(Math.abs(t)) - Math.abs(t));
		}
	...
	void buildArcLengths() {
		...
		for (float[] arcLength : ArcLengths) {
			float[] thisSeg = arcLength;
