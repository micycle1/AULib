...
public class AUField {
		...
		for (int y = 0; y < h; y++) {
			...
			for (int x = 0; x < w; x++) {
				...
				switch (_valueType) {
					...
					default :
					case FIELD_AVG_RGB :
						v = (((c >> 16) & 0xFF) + ((c >> 8) & 0xFF) + (c & 0xFF)) / 3.f;
						break;
					case FIELD_LUM :
						v = (.3f * ((c >> 16) & 0xFF)) + (.59f * ((c >> 8) & 0xFF)) + (.11f * (c & 0xFF));
						break;
				}
				z[y][x] = v;
			}
	...
	// the offset in x and y, the mask (and offset), and the destination pixels (and
	// width and height)
		...
		for (int y = 0; y < h; y++) {
			...
			for (int x = 0; x < w; x++) {
				...
				if (_mask == null) {
					...
					// The following lines are from the old days when I had a separate routine just
					// for writing
					// to PGraphics objects. We don't need this now, but I'm keeping it here for a
					// while in case
	...
	// And now the nine different ways we can write to pixels! There are three basic
	// calls:
	...
	// So that's a total of nine calls. They just call writeToPixels() with slightly
	// different arguments.

	public void toPixels(float _dx, float _dy) {
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), null, Math.round(_dx), Math.round(_dy), theSketch.pixels, theSketch.width,
				theSketch.height);
	...
	public void toPixels(float _dx, float _dy, AUField _mask) {
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, Math.round(_dx), Math.round(_dy), theSketch.pixels, theSketch.width,
				theSketch.height);
		theSketch.updatePixels();
	}
	...
	public void toPixels(float _dx, float _dy, AUField _mask, float _mx, float _my) {
		theSketch.loadPixels();
		writeToPixels(Math.round(_dx), Math.round(_dy), _mask, Math.round(_mx), Math.round(_my), theSketch.pixels, theSketch.width,
				theSketch.height);
		theSketch.updatePixels();
	}
