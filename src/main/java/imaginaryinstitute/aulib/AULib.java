...
public class AULib implements PConstants {
	...
	// When value is positive, return value % modulus. When value is negative,
	// return
	...
	// flip things around so input values in the interval [0, modulus) return
	// unchanged,
	// just as when modulus is positive.
	// see
	// http://www.imaginary-institute.com/blog/2015/07/11/a-practical-replacement-for-modulo

	float wrap(float v, float m) {
		if (m > 0) {
			if (v < 0) {
				v = m - ((-v) % m); // get negative v into region [0, m)
			}
		} else {
			m = -m; // the positive value is easier to work with
			if (v < 0) {
				v += m * (1 + Math.floor(-v / m)); // add m enough times so v > 0
			}
	...
	// given time in [0,1] return [0,1] showing where we are in the lifetime of this
	// object.
	// 0 <= startTime < 1, startTime < endTime < 2
	// We actually want to return 2 pieces of information: the new time and whether
	// you're
	// inside the range. But we can only return one data item, sigh. Since the new
	// time will
	...
	static float boxStyleDistanceToP(float ax, float ay, float bx, float by, float px, float py) {
		...
		if (refDist < AB_tolerance) {
			refDist = 1;
		}
		if (distpv < distpk) {
			return AUMisc.jdist(ax, ay, vx, vy) / refDist;
		}
	...
	static float angleStyleDistanceToP(float ax, float ay, float bx, float by, float px, float py) {
		...
		while (thetaP > thetaB) {
			thetaP -= (float) (2 * Math.PI);
		}
		if (Math.abs(thetaB - thetaP) > Math.PI) {
			thetaP += (float) (2 * Math.PI);
		}
	...
	static float getTOnLineNearestP(float ax, float ay, float bx, float by, float px, float py) {
		// quick and easy, find intersection of line (a,b) and its perpendicular through
		// p
	...
	static float intersectLines(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4) {
		...
		float t = 0;
		if (tden != 0) {
			t = tnum / tden; // tden should never be 0, but always be safe
		}
		return t;
	}

	static float ngonStyleDistanceToP(int n, float ax, float ay, float bx, float by, float px, float py) {
		// Same vector algebra comment as for the plus version. So much code for simple
		// vector algebra.
		if (n < 3) {
			return linearStyleDistanceToP(ax, ay, bx, by, px, py);
		}
		boolean foundAny = false;
		float edgeDist = (float) (AUMisc.jdist(ax, ay, bx, by) * Math.cos((2 * Math.PI) / (2 * n)));
		if (edgeDist == 0) {
			return (0); // A=B, so nothing interesting here
		}
	...
	public static float ease(int _easeType, float _t) {
		// This would be so much nicer with function pointers or functions as
		// parameters. But we
		// can't do that in Processing (or in Java), so I just repeat the same
		// in/out/inout format
	...
	// The industry standard for g is 1.70158. I don't know where that comes from,
	// or
	// why it's so precise. Anything near 1.7 produces a visually indistinguishable
	// result.
	...
	// This version of elastic really runs does from [0,1]. Rarely worth the extra
	// effort.
	...
	public static float wave(int _waveType, float _t, float _a) {
		...
		if (t == 0) {
			symt = (float) .9999999; // funky edge case because f(0) often not f(1)
		}
		float v = 0;
		...
		switch (_waveType) {
			...
			case WAVE_BIAS :
				...
				// 0 to 1. a = 0 = flat w/jump at 1, .25=bowed down, .5=flat, .75=bowed up,
				// 1=jump to 1
				if (a == 0) {
					v = (t < 1) ? 0 : 1;
				} else {
